...
create table if not exists public.map_nano_commercial_requests (
  ...
  product_key text not null default 'map.nano'
    check (product_key = 'map.nano'),
  plan_key text not null
    check (plan_key in ('essential', 'professional', 'facility', 'institutional', 'project')),
  request_type text not null
    check (request_type in ('new_license', 'upgrade', 'institutional_quote', 'project_access', 'demo')),
  status text not null default 'pending'
    check (status in ('pending', 'in_review', 'resolved', 'declined', 'cancelled')),
  contact_name text not null
    check (char_length(btrim(contact_name)) between 2 and 160),
  contact_email text not null
    check (char_length(btrim(contact_email)) between 3 and 320 and contact_email ~* '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$'),
  organization_name text not null
    check (char_length(btrim(organization_name)) between 2 and 240),
  country text not null
    check (char_length(btrim(country)) between 2 and 120),
  estimated_users integer not null
    check (estimated_users between 1 and 100000),
  analysis_volume text not null
    check (analysis_volume in ('under_100', '100_to_1000', 'over_1000', 'unknown')),
  message text
...
drop policy if exists map_nano_commercial_requests_deny_direct_access
  on public.map_nano_commercial_requests;
...
create index if not exists map_nano_commercial_requests_account_created_idx
  on public.map_nano_commercial_requests (account_id, created_at desc)
  where account_id is not null;

create unique index if not exists map_nano_commercial_requests_one_open_change
  on public.map_nano_commercial_requests (
    product_key,
    (coalesce(account_id, requester_id)),
    plan_key,
    request_type
  )
...
drop trigger if exists touch_map_nano_commercial_request
  on public.map_nano_commercial_requests;
...
create or replace function private.is_map_nano_commercial_request_manager(p_actor_id uuid)
returns boolean
language sql
stable
security definer
...
revoke all on function private.is_map_nano_commercial_request_manager(uuid)
  from public, anon, authenticated;
grant execute on function private.is_map_nano_commercial_request_manager(uuid)
...
begin
  ...
  insert into public.map_nano_commercial_requests (
    requester_id,
    account_id,
    plan_key,
    request_type,
...
as $$
  select *
  from private.create_my_map_nano_commercial_request(
    p_plan_key,
    p_request_type,
    p_contact_name,
    p_contact_email,
...
revoke all on function public.create_my_map_nano_commercial_request(text, text, text, text, text, text, integer, text, text, uuid)
  from public, anon;
grant execute on function public.create_my_map_nano_commercial_request(text, text, text, text, text, text, integer, text, text, uuid)
  to authenticated, service_role;
...
as $$
  ...
  accessible_requests as (
    select
      request.*,
      (
        request.requester_id = actor.user_id
        or exists (
          select 1
          from public.license_account_members membership
  ...
  select
    ...
    request.status in ('pending', 'in_review')
      and request.is_requester_or_account_manager
  from accessible_requests request
  order by request.created_at desc, request.id desc;
$$;

revoke all on function private.get_my_map_nano_commercial_requests()
  from public, anon, authenticated;
grant execute on function private.get_my_map_nano_commercial_requests()
  to authenticated, service_role;
...
revoke all on function private.cancel_my_map_nano_commercial_request(uuid, text)
  from public, anon, authenticated;
grant execute on function private.cancel_my_map_nano_commercial_request(uuid, text)
  to authenticated, service_role;
