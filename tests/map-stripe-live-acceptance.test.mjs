...
test("live zero-dollar acceptance is isolated from public Checkout and fail-closed", async () => {
  ...
  assert.match(acceptance, /STRIPE_LIVE_ACCEPTANCE_COUPON_ID/);
  assert.match(acceptance, /expand: \["applies_to"\]/);
  assert.match(acceptance, /coupon\.percent_off === 100/);
  assert.match(acceptance, /coupon\.duration === "once"/);
  ...
  assert.doesNotMatch(checkout, /STRIPE_LIVE_ACCEPTANCE_COUPON_ID/);
