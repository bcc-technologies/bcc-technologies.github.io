(() => {
  ...
  const formatUsd = value => Number.isFinite(value) ? `US${USD.format(value)}` : "Requiere cotización";
  const formatNumber = value => Number.isFinite(value) ? NUMBER.format(value) : "No calculable";
  const practicalImages = value => Number.isFinite(value) ? `${Math.ceil(Math.max(0, value)).toLocaleString("en-US")} imágenes/mes` : "No calculable";
  ...
  function calculatorMarkup() {
    ...
    return `<section class="map-savings-calculator" id="savings-calculator" aria-labelledby="savings-calculator-title">
  ...
  function render(root, options = {}) {
    ...
    const renderResults = () => {
      ...
      const plan = catalog.planById(state.planId);
      const payback = result.paybackMonths === null ? "No se alcanza el equilibrio con estos supuestos." : result.paybackMonths > 12 ? "Más de 12 meses" : `${formatNumber(result.paybackMonths)} meses`;
      const net = result.annualNetSavings === null ? "Requiere cotización" : formatUsd(result.annualNetSavings);
      const roi = result.roiPercent === null ? "Requiere cotización" : `${formatNumber(result.roiPercent)} %`;
      const grossSavingsHelp = result.annualReplaceableCosts > 0
        ...
        : "Valor estimado del tiempo técnico liberado.";
      resultRoot.innerHTML = [
        resultMetric({ id: "hours-saved", label: "Horas técnicas ahorradas", value: `${formatNumber(result.annualHoursSaved)} horas/año`, help: `Equivale aproximadamente a ${formatNumber(result.annualHoursSaved / 8)} jornadas de 8 horas.` }),
      ...
      assessmentRoot.innerHTML = `<strong>${escapeHtml(assessment.title)}</strong><p>${escapeHtml(assessment.text)}</p>${result.annualNetSavings !== null && result.annualNetSavings < 0 ? "<p>Considera revisar los supuestos, Essential, MAP-Nano Project o análisis asistido; otros beneficios deben evaluarse por separado.</p>" : ""}`;
      ...
      recommendationRoot.innerHTML = isEvaluatedPlanSufficient
        ? `<strong>Capacidad declarada</strong><p>El plan evaluado, ${escapeHtml(evaluatedPlanName)}, cubre las necesidades indicadas: ${escapeHtml(capacity.reasons.join(" y "))}. Esta es una orientación comercial; confirma funciones, usuarios y soporte.</p>`
        : `<strong>Capacidad declarada</strong><p>Estás evaluando ${escapeHtml(evaluatedPlanName)}. Con las necesidades indicadas, ${escapeHtml(capacityPlanName)} cubriría la capacidad mínima: ${escapeHtml(capacity.reasons.join(" y "))}. Puedes recalcular este escenario con ese nivel antes de solicitarlo.</p>`;
      diagnosticBadgesRoot.innerHTML = `<span class="map-savings-diagnostic-badge is-${escapeHtml(assessment.grade)}">${escapeHtml(assessment.badge)}</span><span class="map-savings-diagnostic-badge is-plan">${escapeHtml(evaluatedPlanName.replace("MAP-Nano ", ""))}</span>`;
      const requestPlan = result.annualNetSavings !== null && result.annualNetSavings < 0 ? "project" : state.planId;
      const mainLabel = requestPlan === "project" ? "Consultar alternativa Project" : state.planId === "institutional" ? "Hablar sobre Institutional" : `Solicitar ${plan?.name.replace("MAP-Nano ", "") || "plan"}`;
      const recommendationAction = capacity.planId !== state.planId
        ? `<button class="btn btn-ghost" type="button" data-savings-use-recommendation="${escapeHtml(capacity.planId)}">Recalcular con ${escapeHtml(capacityPlanName.replace("MAP-Nano ", ""))}</button>`
        : "";
      actionsRoot.innerHTML = `<a class="btn btn-primary" href="${escapeHtml(requestUrl(requestPlan, result))}" data-savings-cta="quote">${escapeHtml(mainLabel)}</a>${recommendationAction}<a class="btn btn-ghost" href="${escapeHtml(requestUrl(state.planId, result, "demo"))}" data-savings-cta="demo">Solicitar una demostración</a>`;
      comparisonRoot.innerHTML = catalog.PLANS.filter(item => Number.isFinite(item.annualPrice)).map(item => {
        ...
        const comparablePayback = comparable.paybackMonths === null ? "No alcanzable" : comparable.paybackMonths > 12 ? "Más de 12 meses" : `${formatNumber(comparable.paybackMonths)} meses`;
    ...
    form.addEventListener("input", event => {
      ...
      if (target.type === "number" && !Number.isFinite(Number(target.value))) {
        feedback.textContent = "Introduce un número válido; mientras se corrige, se usa 0 para evitar resultados engañosos.";
      } else {
    ...
    root.addEventListener("click", event => {
      ...
      if (recommendationAction) {
        ...
        if (recommendedPlanId !== state.planId) {
          ...
          update();
          feedback.textContent = `Escenario recalculado con ${catalog.planById(recommendedPlanId)?.name || "el plan recomendado"}.`;
      ...
      if (event.target.closest("[data-savings-share]")) {
        ...
        const done = () => { feedback.textContent = "Enlace del escenario copiado. No incluye datos personales."; };
        if (navigator.clipboard?.writeText) navigator.clipboard.writeText(url).then(done).catch(() => { feedback.textContent = "Copia la URL de esta página para compartir el escenario."; });
        else feedback.textContent = "Copia la URL de esta página para compartir el escenario.";
