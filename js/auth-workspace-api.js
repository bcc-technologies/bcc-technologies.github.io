(() => {
  ...
  function abortable(query, signal) {
  ...
  function createWorkspaceApi(deps) {
    async function handle(path, options = {}) {
      ...
      if (path === "/api/workspace/push-subscriptions" && options.method === "DELETE") {
        ...
        const { error } = await abortable(supabase
          .from("workspace_push_subscriptions")
          .delete()
          .eq("endpoint", body.endpoint), options.signal);
        if (error) throw error;
        return handled({ ok: true });
      }

      if (path === "/api/workspace/task-collaborators" && (!options.method || options.method === "GET")) {
        const collaborators = await loadWorkspaceTaskCollaborators(supabase, options.signal);
        return handled({ ok: true, collaborators });
      }

      if (path === "/api/workspace/tasks" && (!options.method || options.method === "GET")) {
        const collaborators = await loadWorkspaceTaskCollaborators(supabase, options.signal).catch(() => []);
        const { data, error } = await abortable(supabase
      ...
      if (workspaceTaskMatch && options.method === "DELETE") {
        const { error } = await abortable(supabase
          .from("workspace_tasks")
          .delete()
          .eq("id", decodeURIComponent(workspaceTaskMatch[1])), options.signal);
        if (error) throw error;
        return handled({ ok: true });
      }
      ...
      if (workspaceEventMatch && options.method === "DELETE") {
        const { error } = await abortable(supabase
          .from("workspace_events")
          .delete()
          .eq("id", decodeURIComponent(workspaceEventMatch[1])), options.signal);
        if (error) throw error;
        return handled({ ok: true });
      }
      ...
      if (path === "/api/workspace/form-responses/me") {
        const { data, error } = await abortable(supabase
          ...
          .order("submitted_at", { ascending: false }), options.signal);
        if (error) throw error;
      ...
      if (responseListMatch) {
        ...
        const { data: profiles, error: profileError } = profileIds.length
          ? await abortable(supabase.from("profiles").select("id, display_name, full_name, email").in("id", profileIds), options.signal)
          : { data: [], error: null };
      ...
      if (responseSubmitMatch && options.method === "POST") {
        ...
        const { data: form, error: formError } = await abortable(supabase
          .from("workspace_forms")
          .select(columns.forms)
          .eq("id", formId)
          .single(), options.signal);
