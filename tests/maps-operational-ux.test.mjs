...
test("client MAP dashboard separates entitlements, commercial access and secondary activity", () => {
  ...
  assert.match(styles, /\.client-license-suite-card\{/);
  ...
  assert.match(styles, /\.client-license-suite-grid\{ display: grid; grid-template-columns: repeat\(3, minmax\(0, 1fr\)\);/);
  assert.match(styles, /\.client-license-suite-card:hover/);
  assert.match(styles, /\.client-license-secondary-disclosure/);
  assert.match(styles, /\.client-license-access-disclosure/);
