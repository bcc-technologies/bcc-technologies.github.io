...
insert into public.map_products (key, name) values
  ('map-nano', 'MAP-Nano'), ('map-bio', 'MAP-Bio'), ('map-med', 'MAP-Med'), ('map-ing', 'MAP-Ing')
...
create table if not exists public.map_licenses (
  ...
  status text not null default 'draft' check (status in ('draft', 'trial', 'active', 'grace', 'suspended', 'expired', 'cancelled')),
...
create index if not exists map_license_events_license_created_idx on public.map_license_events (license_id, created_at desc);
...
create trigger touch_map_license_updated_at before update on public.map_licenses
  ...
  select exists (
    ...
    where profile.id = auth.uid()
      and (
        profile.role = 'admin'
        or exists (
          select 1 from public.workspace_role_definitions definition
...
create policy "licensed staff read organizations" on public.organizations for select to authenticated
...
create policy "licensed staff read products" on public.map_products for select to authenticated
using ((select private.has_license_permission('licenses:view')));
...
create policy "licensed staff read licenses" on public.map_licenses for select to authenticated
using ((select private.has_license_permission('licenses:view')));
...
create policy "licensed staff read entitlements" on public.map_license_entitlements for select to authenticated
using ((select private.has_license_permission('licenses:view')));
...
create policy "license auditors read events" on public.map_license_events for select to authenticated
...
declare
  ...
  seat_count integer;
begin
  if not private.has_license_permission('licenses:manage') then raise exception 'Permiso insuficiente'; end if;
  if organization_name = '' or length(organization_name) > 120 then raise exception 'Organización inválida'; end if;
  if email_address !~* '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$' or length(email_address) > 254 then raise exception 'Correo de contacto inválido'; end if;
  if jsonb_typeof(payload->'products') is distinct from 'array' then raise exception 'Selecciona al menos un producto MAP'; end if;
  select array_agg(distinct value order by value) into product_names
  from jsonb_array_elements_text(payload->'products') product(value);
  if coalesce(cardinality(product_names), 0) = 0 or exists (
    select 1 from unnest(product_names) selected(name)
    where not exists (select 1 from public.map_products product where product.name = selected.name and product.enabled)
  ) then raise exception 'Producto MAP inválido'; end if;
  begin
    seat_count := coalesce(nullif(payload->>'seats', '')::integer, 1);
  exception when invalid_text_representation then
    raise exception 'Cantidad de asientos inválida';
  end;
  if seat_count not between 1 and 100000 then raise exception 'Cantidad de asientos inválida'; end if;
  insert into public.organizations (name) values (organization_name)
  ...
  insert into public.map_licenses (
    organization_id, contact_email, plan, seats, status, platform, starts_at, ends_at, created_by
  ) values (
    target_organization_id, email_address, coalesce(nullif(payload->>'plan', ''), 'Equipo'), seat_count,
    coalesce(nullif(payload->>'status', ''), 'draft'), coalesce(nullif(payload->>'platform', ''), 'Web'),
    coalesce(nullif(payload->>'startsAt', '')::date, current_date), nullif(payload->>'endsAt', '')::date, auth.uid()
  ) returning id into target_license_id;
  insert into public.map_license_entitlements (license_id, product_id)
  select target_license_id, product.id from public.map_products product where product.name = any(product_names);
...
begin
  if not private.has_license_permission('licenses:manage') then raise exception 'Permiso insuficiente'; end if;
  if next_status not in ('draft', 'trial', 'active', 'grace', 'suspended', 'expired', 'cancelled') then raise exception 'Estado inválido'; end if;
  select to_jsonb(license) into before_row from public.map_licenses license where license.id = target_license_id for update;
  if not found then raise exception 'Licencia no encontrada'; end if;
  update public.map_licenses set status = next_status where id = target_license_id returning to_jsonb(map_licenses) into after_row;
  insert into public.map_license_events (license_id, actor_id, event_type, before_state, after_state)
  values (target_license_id, auth.uid(), 'license.status_changed', before_row, after_row);
end;
$$;
...
revoke all on public.organizations, public.map_products, public.map_licenses, public.map_license_entitlements, public.map_license_events from public, anon;
grant select on public.organizations, public.map_products, public.map_licenses, public.map_license_entitlements, public.map_license_events to authenticated;
