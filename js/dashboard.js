document.addEventListener("DOMContentLoaded", async () => {
  const user = await window.BCCAuth.requireAuth();
  if (!user) return;
  ...
  if (adminLink) adminLink.hidden = !user.permissions.includes("admin:view");

  const cmsLink = document.querySelector("[data-cms-link]");
  if (cmsLink) cmsLink.hidden = !user.permissions.includes("cms:access");
