...
create index if not exists map_license_assignments_license_status_idx on public.map_license_assignments (license_id, status);
create index if not exists map_license_assignments_user_status_idx on public.map_license_assignments (user_id, status);
...
create trigger sync_map_license_used_seats after insert or update of status or delete on public.map_license_assignments
...
create policy "licensed staff read assignments" on public.map_license_assignments for select to authenticated
...
begin
  if not private.has_license_permission('licenses:assign') then raise exception 'Permiso insuficiente'; end if;
  return query
  select profile.id,
    coalesce(nullif(profile.display_name, ''), nullif(profile.full_name, ''), split_part(profile.email, '@', 1), 'Usuario'),
...
returns table (id uuid, user_id uuid, name text, email text, status text, assigned_at timestamptz, revoked_at timestamptz)
language plpgsql stable security definer set search_path = public, pg_temp as $$
begin
  if not private.has_license_permission('licenses:view') then raise exception 'Permiso insuficiente'; end if;
  if not exists (select 1 from public.map_licenses where map_licenses.id = target_license_id) then raise exception 'Licencia no encontrada'; end if;
  return query
  select assignment.id, assignment.user_id,
    coalesce(nullif(profile.display_name, ''), nullif(profile.full_name, ''), split_part(profile.email, '@', 1), 'Usuario'),
    profile.email, assignment.status, assignment.assigned_at, assignment.revoked_at
  from public.map_license_assignments assignment
  join public.profiles profile on profile.id = assignment.user_id
  where assignment.license_id = target_license_id and assignment.status = 'active'
  order by assignment.assigned_at;
end;
$$;

create or replace function private.assign_map_license_user(target_license_id uuid, target_user_id uuid)
returns table (id uuid, user_id uuid, name text, email text, status text, assigned_at timestamptz, revoked_at timestamptz)
...
begin
  if not private.has_license_permission('licenses:assign') then raise exception 'Permiso insuficiente'; end if;
  select * into license_row from public.map_licenses where map_licenses.id = target_license_id for update;
  if not found then raise exception 'Licencia no encontrada'; end if;
  if license_row.status not in ('trial', 'active', 'grace') then raise exception 'La licencia debe estar activa, en prueba o en gracia'; end if;
  if license_row.used_seats >= license_row.seats then raise exception 'No hay asientos disponibles'; end if;
  if not exists (select 1 from public.profiles where profiles.id = target_user_id) then raise exception 'Usuario no encontrado'; end if;
  if exists (
    select 1 from public.map_license_assignments
    where license_id = target_license_id and user_id = target_user_id and status = 'active'
  ) then raise exception 'El usuario ya tiene un asiento en esta licencia'; end if;
  insert into public.map_license_assignments (license_id, user_id, status, assigned_by, assigned_at, revoked_at, updated_at)
  values (target_license_id, target_user_id, 'active', auth.uid(), now(), null, now())
  on conflict (license_id, user_id) do update
  set status = 'active', assigned_by = auth.uid(), assigned_at = now(), revoked_at = null, updated_at = now()
  ...
  values (target_license_id, auth.uid(), 'license.seat_assigned', jsonb_build_object('assignmentId', target_assignment_id, 'userId', target_user_id));
  return query
  select assignment.id, assignment.user_id,
    coalesce(nullif(profile.display_name, ''), nullif(profile.full_name, ''), split_part(profile.email, '@', 1), 'Usuario'),
    profile.email, assignment.status, assignment.assigned_at, assignment.revoked_at
...
create or replace function private.revoke_map_license_user(target_license_id uuid, target_user_id uuid)
returns void language plpgsql security definer set search_path = public, pg_temp as $$
declare target_assignment_id uuid;
begin
  if not private.has_license_permission('licenses:assign') then raise exception 'Permiso insuficiente'; end if;
  perform 1 from public.map_licenses where map_licenses.id = target_license_id for update;
  if not found then raise exception 'Licencia no encontrada'; end if;
  ...
  insert into public.map_license_events (license_id, actor_id, event_type, before_state)
  values (target_license_id, auth.uid(), 'license.seat_revoked', jsonb_build_object('assignmentId', target_assignment_id, 'userId', target_user_id));
end;
$$;
...
$$;
create or replace function public.assign_map_license_user(target_license_id uuid, target_user_id uuid)
returns table (id uuid, user_id uuid, name text, email text, status text, assigned_at timestamptz, revoked_at timestamptz)
...
$$;
create or replace function public.revoke_map_license_user(target_license_id uuid, target_user_id uuid)
