    ...
    async function handle(path, options = {}) {
      ...
      if (path === "/api/admin/users") {
        ...
        const [{ data, error }, customRoles] = await Promise.all([
          abortable(supabase.from("profiles").select("*").order("created_at", { ascending: false }), options.signal),
          loadWorkspaceRoleDefinitions(supabase, options.signal)
        ]);
      ...
      if (path === "/api/admin/roles" && (!options.method || options.method === "GET")) {
        await requireUsersManage();
        const customRoles = await loadWorkspaceRoleDefinitions(supabase, options.signal);
      ...
      if (path === "/api/admin/roles" && options.method === "POST") {
        await requireUsersManage();
        const existing = await loadWorkspaceRoleDefinitions(supabase, options.signal);
        ...
        if (error) throw error;
        const roles = await loadWorkspaceRoleDefinitions(supabase, options.signal);
      ...
      if (roleDefinitionMatch && (options.method === "PATCH" || options.method === "DELETE")) {
        ...
        if (options.method === "DELETE") {
          const { error } = await abortable(supabase.from("workspace_role_definitions").delete().eq("id", id), options.signal);
          if (error) throw error;
          const roles = await loadWorkspaceRoleDefinitions(supabase, options.signal);
      ...
      if (roleMatch && options.method === "PATCH") {
        ...
        const customRoles = await loadWorkspaceRoleDefinitions(supabase, options.signal);
        const { error } = await abortable(supabase.rpc("set_user_access", {
